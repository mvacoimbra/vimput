...
export function processKey(state: VimState, key: string): VimState {
  ...
  if (state.mode === 'insert') {
    if (key === 'Escape') {
      ...
      return {
        ...state,
        mode: 'normal',
        cursor: { ...state.cursor, column: clampColumn(currentLine, newColumn, 'normal') },
      }
    }
    ...
    if (key.length === 1) {
      const newLine = currentLine.slice(0, state.cursor.column) + key + currentLine.slice(state.cursor.column)
      lines[state.cursor.line] = newLine
      return {
        ...state,
        text: joinLines(lines),
  ...
  if (state.mode === 'visual') {
    ...
    if (key === 'y') {
      ...
      return { ...state, mode: 'normal', yankBuffer: yanked, visualStart: null }
    }
    if (key === 'd' || key === 'x') {
  ...
  if (state.mode === 'normal') {
    ...
    if (key === 'a') {
      ...
      return { ...state, mode: 'insert', cursor: { ...state.cursor, column: newColumn } }
    }
    if (key === 'I') {
    ...
    if (key === 'A') {
      return { ...state, mode: 'insert', cursor: { ...state.cursor, column: currentLine.length } }
    }
    if (key === 'o') {
    ...
    if (key === 'x') {
      if (currentLine.length > 0) {
        ...
        return {
          ...state,
          text: joinLines(lines),
          cursor: { ...state.cursor, column: clampColumn(newLine, state.cursor.column, 'normal') },
        }
      }
      return state
    }
    ...
    if (key === 'd' && state.commandBuffer === 'd') {
      ...
      return {
        ...state,
        text: joinLines(lines),
        cursor: { line: newLine, column: clampColumn(lines[newLine] || '', 0, 'normal') },
        yankBuffer: yanked,
        commandBuffer: '',
      }
    }
    ...
    if (key === 'p') {
      if (state.yankBuffer.includes('\n') || !state.yankBuffer) {
        ...
        // Character paste
        const newLine = currentLine.slice(0, state.cursor.column + 1) + state.yankBuffer + currentLine.slice(state.cursor.column + 1)
        lines[state.cursor.line] = newLine
        return {
          ...state,
          text: joinLines(lines),
          cursor: { ...state.cursor, column: state.cursor.column + state.yankBuffer.length },
        }
      }
    }
    if (key === 'P') {
      if (state.yankBuffer.includes('\n') || !state.yankBuffer) {
        ...
        const newLine = currentLine.slice(0, state.cursor.column) + state.yankBuffer + currentLine.slice(state.cursor.column)
        lines[state.cursor.line] = newLine
        return {
          ...state,
          text: joinLines(lines),
...
function handleNormalMovement(state: VimState, key: string): Partial<VimState> {
  ...
  switch (key) {
    ...
    case 'ArrowRight':
      return {
        cursor: { ...state.cursor, column: clampColumn(currentLine, state.cursor.column + 1, mode) },
      }
    case 'j':
    case 'ArrowDown':
      if (state.cursor.line < lines.length - 1) {
        ...
        return {
          cursor: { line: newLine, column: clampColumn(lines[newLine] || '', state.cursor.column, mode) },
        }
      }
      break
    case 'k':
    ...
    case '$':
      return { cursor: { ...state.cursor, column: Math.max(0, currentLine.length - 1) } }
    case 'g':
    ...
    case 'G':
      ...
      return { cursor: { line: lastLine, column: clampColumn(lines[lastLine] || '', 0, mode) } }
  }
...
function moveToNextWord(state: VimState): Partial<VimState> {
  ...
  return { cursor: { line, column: clampColumn(lines[line] || '', column, state.mode) } }
}
...
function getVisualSelection(state: VimState): string {
  ...
  const [s, e] = start.line < end.line || (start.line === end.line && start.column <= end.column)
    ? [start, end]
    : [end, start]
...
function deleteVisualSelection(state: VimState): VimState {
  ...
  return {
    ...
    mode: 'normal',
    text: joinLines(lines),
    cursor: { line: s.line, column: clampColumn(lines[s.line] || '', s.column, 'normal') },
    yankBuffer: yanked,
    visualStart: null,
  }
}
